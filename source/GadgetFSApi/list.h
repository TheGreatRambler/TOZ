...
#define usb_gadget_list_for_each_entry(pos, head, member) for (pos = usb_gadget_list_entry((head)->next, typeof(*pos), member); pos->member.next, &pos->member != (head); pos = usb_gadget_list_entry(pos->member.next, typeof(*pos), member))
...
static inline void __usb_gadget_list_add(struct usb_gadget_list_head* new, struct usb_gadget_list_head* prev, struct usb_gadget_list_head* next) {
	next->prev = new;
	new->next = next;
	new->prev = prev;
	prev->next = new;
...
static inline void usb_gadget_list_add(struct usb_gadget_list_head* new, struct usb_gadget_list_head* head) {
	__usb_gadget_list_add(new, head, head->next);
}

static inline void usb_gadget_list_add_tail(struct usb_gadget_list_head* new, struct usb_gadget_list_head* head) {
